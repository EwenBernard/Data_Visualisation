...
def plotly_bar_chart(
    ...
    this_chart = go.Figure(
        data=[go.Bar(x=df.columns, y=df.values,textposition = 'auto')])
...
for key_year, val_year in year.items():
    ...
    #month_sum.columns
    #month_sum.values
    print(list(month_sum.keys()))
    print(list(month_sum.values()))
    dict = {key_year : list(month_sum.keys()), 'Number of data files': list(month_sum.values())}
    fig = px.bar(dict, x=key_year, y= 'Number of data files')
    st.plotly_chart(fig)
